...
async function monitorTokenRSI(page, tokenAddress) {
  ...
  const intervalId = setInterval(async () => {
    try {
      ...
      let rsiValue = null;
      let marketCapValue = null;
      let cciValue = null; // {{ Added for CCI }}
      ...
      if (tradingViewFrame) {
        try {
          ...
          const scrapedData = await tradingViewFrame.evaluate(() => {
            ...
            let cciValue = null; // {{ Added for CCI }}
            let cciDebugText = 'CCI(20) debug (monitorTokenRSI): '; // {{ Added for CCI }}
                ...
                if (cciValue !== null) {
                    ...
                    if (!potentialIndicators.length) {
                        cciDebugText += 'No .item-l31H9iuA.study-l31H9iuA indicators found. ';
                         ...
                         const firstIndicator = potentialIndicators[0]; // Check first for debug
                         if (firstIndicator) {
                            const firstMainTitle = firstIndicator.querySelector('.titlesWrapper-l31H9iuA .mainTitle-l31H9iuA .title-l31H9iuA');
                            const firstDescTitle = firstIndicator.querySelector('.titlesWrapper-l31H9iuA .descTitle-l31H9iuA .title-l31H9iuA');
                            if (firstMainTitle) cciDebugText += `First indicator main title: "${firstMainTitle.textContent.trim()}". `;
                            if (firstDescTitle) cciDebugText += `First indicator desc title: "${firstDescTitle.textContent.trim()}". `;
                         }
                    }
                }
            } catch (e) {
                cciDebugText += `Error during CCI(20) extraction: ${e.message}. `;
            }
            // {{ End of CCI(20) Scraping Logic }}
            
            return { rsiValue, marketCapValue, cciValue, rsiDebugText, mcDebugText, cciDebugText }; // {{ Added cciValue, cciDebugText }}
          });

          rsiValue = scrapedData.rsiValue;
          marketCapValue = scrapedData.marketCapValue;
          cciValue = scrapedData.cciValue; // {{ Added for CCI }}


          // Log raw scraped values and debug text if null
          const rawScrapedLog = `${tokenAddress}: Raw scraped in monitor - RSI: ${rsiValue === null ? 'N/A' : rsiValue}, MC: ${marketCapValue === null ? 'N/A' : marketCapValue}, CCI(20): ${cciValue === null ? 'N/A' : cciValue}`; // {{ Added CCI to log }}
          ...
          if (cciValue === null) { // {{ Added for CCI }}
            console.log(`${tokenAddress}: ${scrapedData.cciDebugText}`); // Log CCI debug text if CCI value is null
          }
