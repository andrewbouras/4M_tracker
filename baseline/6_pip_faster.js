...
async function monitorTokenRSI(page, tokenAddress) {
  ...
  let canBuyThisDip = false; // True if a dip has started and we haven't bought on its first rise yet
  let lowestRsiOfCurrentDip = null;
  let lastRsiValue = null;
  
  // Other existing state variables:
  // let isRsiBelow25 = false; // This will be effectively replaced by inSub30Dip logic
  // let hasSentBuyRequest = false; // This will be effectively replaced by canBuyThisDip
  let noRsiDataCount = 0;
  // let rsiTrend = 'unknown'; // Can be simplified with lastRsiValue comparison
  // let lowestRsiValue = null; // Replaced by lowestRsiOfCurrentDip
  let consecutiveNaRsiCount = 0;
  let consecutiveInvalidRsiCount = 0;
  let lastMarketCapValueString = null;
  let lastMarketCapChangeTime = null;
  let pipFetchCounter = 0;
  const PIP_FETCH_INTERVAL_COUNT = 1; // {{ MODIFIED: Was 25, now 1 for 200ms fetch }}
  ...
  const intervalId = setInterval(async () => {
    ...
    try {
      ...
      let rsiValue = null;
      let marketCapValue = null;
      // cciValue is already declared outside

      const scrapedData = await tradingViewFrame.evaluate(() => {
        // ... (existing scraping logic for RSI, MC, CCI - remains unchanged) ...
        let rsiValue = null;
      ...
      // cciValue is assigned directly by scrapedData.cciValue within the interval scope if needed elsewhere in interval
      // For the buy signal, we primarily need RSI, MC, PIP.

      const rawScrapedLog = `${tokenAddress}: Raw scraped in monitor - RSI: ${rsiValue === null ? 'N/A' : rsiValue}, MC: ${marketCapValue === null ? 'N/A' : marketCapValue}, CCI(20): ${scrapedData.cciValue === null ? 'N/A' : scrapedData.cciValue}`;
      ...
      if (rsiValue) {
        if (rsiValue.includes('K') || rsiValue.includes('M') || rsiValue.includes('B')) {
          ...
          // rsiValue = null; // Keep rsiValue as is for logging, rsiNumeric will be null
        } else {
          ...
          if (isNaN(parsedFloat) || parsedFloat < 0 || parsedFloat > 100) {
            ...
            rsiNumeric = parsedFloat; // Assign to rsiNumeric here
            consecutiveInvalidRsiCount = 0;
          }
        }
      }

      // ... (existing consecutiveInvalidRsiCount handling logic - remains unchanged) ...
      ...
      // ... (existing consecutiveNaRsiCount handling logic for when rsiNumeric is null - remains unchanged) ...
      if (rsiNumeric === null) { // Check rsiNumeric instead of rsiValue directly here
        consecutiveNaRsiCount++;
        console.log(`${tokenAddress}: No valid RSI data (numeric) (count: ${consecutiveNaRsiCount})`);
        // ... (attempts to fix, blacklist if persistent - this logic remains)
      ...
      // Check token readiness (RSI and MC working for general status, not for buy signal decision here)
      // This function updates tokensReadyStatus, which is fine.
      ...
      if (marketCapValue) {
        ...
        if (mcMatch) {
          ...
          if (mcNumeric < 25000) {
            ...
            logDetailedError(tokenAddress, 'LOW_MARKET_CAP', `Market cap below 25K`, `Value: ${marketCapValue}`);
            clearInterval(intervalId);
      ...
      // Get PIP value from status for the check
      ...
      } else if (typeof pipString === 'number') { // If it was already stored as a number
          pipNumericForCheck = pipString;
      }
      // If pipString is "Error" or "N/A", pipNumericForCheck remains null.

      // -------- START: New Trading Signal Logic --------
      if (rsiNumeric !== null) { // Ensure rsiNumeric is valid before using in trading logic
          // Manage dip state
          if (rsiNumeric < 30) {
              if (!inSub30Dip) { // RSI just entered sub-30 zone
                  inSub30Dip = true;
                  canBuyThisDip = true; // New dip, new buy opportunity
              ...
              } else { // Still in the sub-30 dip
                  if (rsiNumeric < lowestRsiOfCurrentDip) {
                      lowestRsiOfCurrentDip = rsiNumeric;
                  }
              }
          } else { // rsiNumeric >= 30
              if (inSub30Dip) { // RSI just exited sub-30 zone
                  ...
                  inSub30Dip = false;
                  // canBuyThisDip is NOT reset here. It's consumed by a buy or reset by a new dip.
                  // lowestRsiOfCurrentDip effectively gets reset when a new dip starts.
              }
          }

          // Check for buy trigger
          if (inSub30Dip && canBuyThisDip && lastRsiValue !== null && rsiNumeric > lastRsiValue) {
              // Conditions: In a sub-30 dip, haven't bought on this dip's rise yet, and RSI is increasing.
              ...
              // Now check concurrent conditions: MC and PIP
              if (mcNumeric !== null && mcNumeric > 25000) { // MC > 25k
                  if (pipNumericForCheck !== null && pipNumericForCheck < 20) { // PIP < 20
                      ...
                      canBuyThisDip = false; // Buy action taken for this dip's rise
                      ...
                      // If you want to send actual buy request:
                      // sendBuyRequest(tokenAddress).then(response => {
                      //     console.log(`${tokenAddress}: Buy request sent. Status: ${response.statusCode}, Response: ${response.data}`);
                      // }).catch(error => {
                      //     console.error(`${tokenAddress}: Error sending buy request: ${error.message}`);
                      // });
          ...
          lastRsiValue = rsiNumeric; // Update lastRsiValue for the next iteration
      } else {
          // rsiNumeric is null (due to invalid/missing raw RSI), so we can't evaluate trading signals.
          // The consecutiveNaRsiCount or consecutiveInvalidRsiCount logic handles this.
          // We should not update lastRsiValue if current rsiNumeric is invalid.
      }
      // -------- END: New Trading Signal Logic --------

      // --- Check for stale Market Cap --- (existing logic remains unchanged)
      if (marketCapValue) {
        if (lastMarketCapValueString === null) {
          ...
          if (lastMarketCapChangeTime) {
            ...
            if (timeUnchanged > 120) {
              ...
              logDetailedError(tokenAddress, 'STALE_MARKET_CAP', `Market Cap unchanged for >120s. Last MC: ${lastMarketCapValueString}`, `Current MC: ${marketCapValue}`);
              clearInterval(intervalId);
      ...
      // {{ ADDITION: Fetch Price Impact (PIP) periodically }} (existing logic remains unchanged)
      pipFetchCounter++;
      if (pipFetchCounter >= PIP_FETCH_INTERVAL_COUNT) { // This will now be true every 200ms
        pipFetchCounter = 0;
        try {
          ...
          const status = tokensReadyStatus.get(tokenAddress) || {};
          status.pipValue = currentPipValue; // Store as string, will be parsed by buy logic
      ...
      // ... (existing error handling in interval - remains unchanged) ...
      if (error.message.includes('detached Frame') || error.message.includes('Target closed')) {
