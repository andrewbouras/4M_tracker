...
async function monitorTokenRSI(page, tokenAddress) {
  ...
  let cciValue = null; // Ensure cciValue is declared if used, though not in current buy logic
  ...
  const intervalId = setInterval(async () => {
    ...
    try {
      ...
      let rsiValue = null; // Renamed from rsiValueFromScrape to avoid confusion
      let marketCapValue = null; // Renamed from marketCapValueFromScrape
      // cciValue is declared outside the interval, its value from scrape will be in scrapedData.cciValue
        ...
        // --- CCI(20) Scraping Logic ---
            ...
            potentialIndicators.forEach(indicator => {
                ...
                if (mainTitleElement && mainTitleElement.textContent.trim() === 'CCI' &&
                    ...
                    if (valueElement) {
                        cciValue = valueElement.textContent.trim(); // Assign to the cciValue in evaluate's scope
                    } else {
                        cciDebugText += 'CCI(20) titles found, but value element not found with specific selector. ';
                    }
                }
            });
            if (cciValue !== null) { // Check cciValue in evaluate's scope
      ...
      rsiValue = scrapedData.rsiValue; // Assign from scrape to interval-scoped rsiValue
      marketCapValue = scrapedData.marketCapValue; // Assign from scrape
      // cciValue (the one declared outside interval) is not directly updated here, 
      // scrapedData.cciValue holds the CCI from this scrape.
      ...
      let rsiNumeric = null;
      if (rsiValue) {
        // ... (RSI parsing and validation to rsiNumeric - remains the same) ...
        if (rsiValue.includes('K') || rsiValue.includes('M') || rsiValue.includes('B')) {
          console.log(`${tokenAddress}: Invalid RSI value detected (${rsiValue}), treating as N/A`);
      ...
      // ... (consecutiveInvalidRsiCount handling - remains the same) ...
      ...
      // ... (consecutiveNaRsiCount handling for rsiNumeric === null - remains the same) ...
      if (rsiNumeric === null) { 
      ...
      let mcNumeric = null;
      if (marketCapValue) {
        // ... (MC parsing to mcNumeric and <25k check - remains the same) ...
      ...
      // -------- START: REVISED Trading Signal Logic --------
      if (rsiNumeric !== null) {
          const rsiIncreasedThisTick = lastRsiValue !== null && rsiNumeric > lastRsiValue;

          // Check for buy trigger first.
          // This uses 'inSub30Dip' which reflects the state *before* this tick's RSI potentially changes it.
          if (inSub30Dip && canBuyThisDip && rsiIncreasedThisTick) {
              ...
              if (mcNumeric !== null && mcNumeric > 25000) {
                  if (pipNumericForCheck !== null && pipNumericForCheck < 20) {
                      ...
                      canBuyThisDip = false; // Consume the buy opportunity for this dip's rise
          ...
          // Now, manage the dip state based on the current rsiNumeric.
          // This will set up 'inSub30Dip' and 'canBuyThisDip' for the *next* interval's buy check.
          if (rsiNumeric < 30) {
              if (!inSub30Dip) { // RSI just dropped below 30
                  inSub30Dip = true;
                  canBuyThisDip = true; // Arm for a buy on a subsequent rise
              ...
              if (inSub30Dip) { // RSI was below 30, now it's risen to >= 30
                  ...
                  inSub30Dip = false;
                  // 'canBuyThisDip' is NOT reset here.
                  // It's either false (if a buy just triggered) or it remains true if the dip ended
                  // without a buy signal (e.g., RSI gapped up, or PIP/MC was bad).
                  // It will be re-armed to true if a *new* dip (RSI < 30) starts.
              }
          }
          lastRsiValue = rsiNumeric; // Update lastRsiValue for the next iteration
      } else {
          // rsiNumeric is null (due to invalid/missing raw RSI), so we can't evaluate trading signals or update lastRsiValue.
          // The consecutiveNaRsiCount or consecutiveInvalidRsiCount logic handles this.
      }
      // -------- END: REVISED Trading Signal Logic --------
